...
import {cameraProfile} from './camera-profile.js';
export function createAtmosphere(renderer,scene,hdr){
 ...
 let lastPosition=new T.Vector3(Infinity,0,0),lastSpan=-1,quality=true,shadowUpdates=0;const lastDirection=new T.Vector3(),direction=new T.Vector3(),right=new T.Vector3(),up=new T.Vector3(),anchor=new T.Vector3(),worldUp=new T.Vector3(0,1,0),warm=new T.Color('#ffc394'),cool=new T.Color('#a7c6f3'),fogNight=new T.Color('#101c30');
 return{sun,get shadowUpdates(){return shadowUpdates;},update(value,camera,aerial,snapshot){
  ...
  // A flight changes the UI mode before the camera arrives. Its actual height must
  // drive the fog and shadow footprint, otherwise every mode switch flashes.
  const profile=cameraProfile(camera.position.y),span=profile.shadowSpan;
  if(direction.distanceToSquared(lastDirection)>.0000005||camera.position.distanceToSquared(lastPosition)>36||Math.abs(span-lastSpan)>.25){
   const step=span*2/(quality?4096:2048);right.crossVectors(worldUp,direction);if(right.lengthSq()<.0001)right.set(1,0,0);right.normalize();up.crossVectors(direction,right).normalize();anchor.set(T.MathUtils.lerp(camera.position.x,50,profile.shadowAnchorBlend),0,T.MathUtils.lerp(camera.position.z,-50,profile.shadowAnchorBlend));const x=Math.round(anchor.dot(right)/step)*step,y=Math.round(anchor.dot(up)/step)*step;sun.target.position.copy(right).multiplyScalar(x).addScaledVector(up,y);sun.position.copy(sun.target.position).addScaledVector(direction,1200);Object.assign(sun.shadow.camera,{left:-span,right:span,top:span,bottom:-span,near:5,far:2400});sun.shadow.camera.updateProjectionMatrix();renderer.shadowMap.needsUpdate=true;lastDirection.copy(direction);lastPosition.copy(camera.position);lastSpan=span;shadowUpdates++;
  }
  sun.intensity=altitude>0?3.6*T.MathUtils.smoothstep(altitude,0,18):snapshot.moonAboveHorizon?.26*snapshot.moonIllumination:0;sun.color.set('#fff1d4').lerp(warm,sunset).lerp(cool,night);hemi.intensity=.32-night*.13;hemi.color.set('#c6dcf0').lerp(cool,night);scene.environmentIntensity=.73-night*.55;scene.fog.color.set('#b6ccdc').lerp(warm,sunset*.2).lerp(fogNight,night);scene.fog.density=profile.fogDensity;renderer.toneMappingExposure=1.04+night*.14;return night;
 },setQuality(high){quality=high;lastSpan=-1;sun.shadow.mapSize.set(high?4096:2048,high?4096:2048);if(sun.shadow.map){sun.shadow.map.dispose();sun.shadow.map=null;}renderer.shadowMap.needsUpdate=true;}};
}
