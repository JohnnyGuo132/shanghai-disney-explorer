...
export function createUnderstory(scene,data,sources,blocked,paths){
 ...
 return{count:positions.length,setQuality:v=>{high=v;last=-1;},update(t,camera){if(last>=0&&t-last<.28)return;if(last>=0&&camera.position.distanceToSquared(lastPosition)<.25)return;last=t;lastPosition.copy(camera.position);const counts=Array(sources.length).fill(0);for(const p of positions){const d=distance([p.x,p.z],[camera.position.x,camera.position.z]),range=high?(p.type<2?83:58):(p.type<2?48:35);if(d>range||camera.position.y>150)continue;const fade=1-T.MathUtils.smoothstep(d,range-16,range);dummy.position.set(p.x,p.y-(1-fade)*.12,p.z);dummy.rotation.set(0,p.angle,0);dummy.scale.setScalar(p.scale*fade);dummy.updateMatrix();for(const mesh of batches[p.type]){mesh.setMatrixAt(counts[p.type],dummy.matrix);mesh.setColorAt(counts[p.type],new T.Color().setHSL(.21,.03,.82+(counts[p.type]%7)*.024));}counts[p.type]++;}batches.forEach((arr,i)=>arr.forEach(m=>{m.count=counts[i];m.instanceMatrix.needsUpdate=true;if(m.instanceColor)m.instanceColor.needsUpdate=true;}));}};
}
