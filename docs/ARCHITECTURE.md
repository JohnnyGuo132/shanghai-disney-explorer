...
线上由 GitHub Pages 直接托管 `dist/`，入口为 [奇境漫游在线版](https://johnnyguo132.github.io/shanghai-disney-explorer/)，访问者无需安装 Node.js。`.github/workflows/pages.yml` 校验后发布 `main` 中的静态文件，不需要后台服务或数据库。首次加载约 30 MB 模型与纹理。

`scripts/serve.mjs` 用 Node.js 内置 HTTP 模块提供本地预览，供开发与本机体验使用；它不会随网站部署到服务器运行。配置与子路径验证见 [部署指南](./DEPLOYMENT.md)。
...
主场景使用项目随附数据；在线版从同一站点加载模型与纹理，不接入票务、实时排队、园区账号或外部素材服务。用户主动打开素材来源链接时会访问对应外部网站。在线内容随 `main` 的成功部署更新，重新打开或刷新页面即可加载站点当前文件；本地副本通过下载新版或更新仓库获取改动。
